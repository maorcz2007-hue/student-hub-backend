...
export async function getDashboardData(req, res, next) {
    try {
        ...
        let avgGrade = 0.0;
        if (grades.length > 0) {
            const sum = grades.reduce((acc, curr) => acc + (curr.score / curr.maxScore) * 4.0, 0);
            gpa = Number((sum / grades.length).toFixed(2));
            const gradeSum = grades.reduce((acc, curr) => acc + (curr.score / curr.maxScore) * 100, 0);
